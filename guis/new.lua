...
function mainapi:CreateGUI()
	...
	-- VainLogo.lua treatment (1:1): blue metallic 5-keypoint gradient + a slow
	...
	logo.RichText = false
	-- White base so the blue gradient defines the colour purely (UIGradient
	...
	logoGrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 61, 154)),
		ColorSequenceKeypoint.new(0.28, Color3.fromRGB(31, 106, 255)),
		ColorSequenceKeypoint.new(0.48, Color3.fromRGB(138, 196, 255)),
		ColorSequenceKeypoint.new(0.68, Color3.fromRGB(31, 106, 255)),
		ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 47, 122)),
	})
...
-- One-time recovery for the poopparty merge's module/option renames: saved
-- settings are keyed by exact Name string (see :Save/:SaveOptions below), so
-- every renamed module/option silently lost its saved config on next load
...
local KILLAURA_LEGACY_OPTIONS = {
	['Dynamic hits'] = 'Dynamic Reach',
	['Attackable check'] = 'Attack Check',
}
...
local function migrateLegacySaveData(savedata)
	...
	if killaura and killaura.Options then
		local opts = killaura.Options
		for oldName, newName in pairs(KILLAURA_LEGACY_OPTIONS) do
			if opts[oldName] and not opts[newName] then
				opts[newName] = opts[oldName]
			end
		end
		-- 'Hitreg' -> 'Hits Per Second' now lives behind a 'Custom Hit Reg' toggle
		-- that previously didn't exist (the old slider was always active), so
		-- also flip that toggle on to preserve the old always-limited behaviour.
		if opts['Hitreg'] and not opts['Hits Per Second'] then
			opts['Hits Per Second'] = opts['Hitreg']
			if not opts['Custom Hit Reg'] then
				opts['Custom Hit Reg'] = {Enabled = true}
			end
		end
		-- Same story: 'Swing time' -> 'Swing Time' now needs 'Custom Swing Time' on.
		if opts['Swing time'] and not opts['Swing Time'] then
			opts['Swing Time'] = opts['Swing time']
			if not opts['Custom Swing Time'] then
				opts['Custom Swing Time'] = {Enabled = true}
			end
		end
	end
end
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('vain/profiles/'..self.Profile..self.Place..'.txt') then
		...
		migrateLegacySaveData(savedata)
	...
	if not inputService.KeyboardEnabled or shared.VainDeveloper then
		...
		fallback.TextColor3 = Color3.fromRGB(31, 106, 255)
...
friends = mainapi:CreateCategoryList({
	...
	-- Fire the update event when the friends list changes so presence notifications
	-- ("X is in the server") and highlights refresh. Without this the list defaulted
	-- to a no-op and adding a friend never notified.
	Function = function()
		if friends.Update then friends.Update:Fire() end
	end,
})
...
function mainapi:UpdateGUI(hue, sat, val, default)
	...
	for i, v in mainapi.Categories do
		if i == 'Main' then
			...
			-- with the system hue here, or the blue logo turns the theme colour.)
