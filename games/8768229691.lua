...
run(function()
	...
	vain:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	vain:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
	...
	local anims, armC0 = vain.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and vain.ThreadFix then
						setthreadidentity(8)
					end
	...
	Viewmodel = vain.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				vain:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
