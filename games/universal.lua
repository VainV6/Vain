...
local function serverHop(pointer, filter)
	...
	if not pointer then
		notif('Vain', 'Searching for an available server.', 2)
	end
...
vain:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport(
			"shared.vainserverhoplist = '"
				.. table.concat(visited, '/')
				.. "'\nshared.vainserverhopprevious = '"
				.. game.JobId
				.. "'"
		)
	end
...
vain.Libraries.string = loadstring(downloadFile('vain/libraries/string.lua'), 'string')()
...
-- pcall'd unlike the libraries above, which are load-bearing for everything
-- here: this one is optional, and it sits at TOP LEVEL of this file, so a
-- download failure (a client pinned to a commit from before troll.lua existed,
-- a GitHub blip) throwing here would abort every module registered below it --
-- the exact "most modules silently vanished" failure this file already guards
-- against elsewhere. Missing library just means no troll commands.
local trolllib
do
	local ok, res = pcall(function()
		return loadstring(downloadFile('vain/libraries/troll.lua'), 'trolllibrary')()
	end)
	trolllib = ok and type(res) == 'table' and res or nil
	if not ok then warn('[Vain] troll library failed to load: '..tostring(res)) end
end
...
run(function()
	...
	-- Notify when a friend or target joins, leaves, or is already here. Always on.
	...
	task.spawn(function()
		...
		task.wait(0.5)
		-- Keep rescanning on a light poll instead of trusting the per-list Update
		-- events alone: a GUI skin may forget to fire them on a mutation, and the
		-- saved profile can populate Friends/Targets before this script connects its
		-- listeners (missing the load-time announce). announced[] dedupes so every
		-- listed player is still announced exactly once.
		while vain.Connections do
			scanServer()
			task.wait(2)
		end
	end)
	...
	vain:Clean(vain.Categories.Friends.Update.Event:Connect(function()
		entitylib.refresh()
	end))
	vain:Clean(vain.Categories.Targets.Update.Event:Connect(function()
		entitylib.refresh()
	end))
	vain:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	vain:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('vain/profiles/whitelist.json') and readfile('vain/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				vain:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and vain.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('vain/profiles/whitelist.json', whitelist.textdata)
				end)
			end
	...
	whitelist.commands = {
		...
		framerate = function(args)
			if #args < 1 or not setfpscap then
				return
			end
...
run(function()
	...
	Radar = vain:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('vain/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(vain.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	SessionInfo = vain:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport(
							"shared.vainsessioninfo = '"
								.. httpService:JSONEncode(vain.Libraries.sessioninfo.Objects)
								.. "'"
						)
					end
				end))
	...
	vain:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if vain.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (vain.gui.AbsoluteSize.X / 2)
    	...
    	Function = function(callback)
    		...
    		if callback then
    			...
    			AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
    				...
    				if rightClicked and not vain.gui.ScaledGui.ClickGui.Visible then
    			...
    			repeat
    				if Mode.Value == 'Tool' then
    					...
    					if mouse1click and (isrbxactive or iswindowactive)() then
    						if not vain.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	RemoteSpy = vain.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			enabled = callback
			if callback then
				print('[RemoteSpy] started, logging remote calls to this console')
...
run(function()
    ...
    Desync = vain.Categories.Blatant:CreateModule({
        Name = 'Desync',
        Function = function(callback)
            if callback then
...
run(function()
    ...
    Spider = vain.Categories.Blatant:CreateModule({
    	Name = 'Spider',
    	Function = function(callback)
    		if callback then
    			if Truss then
...
run(function()
    ...
    Swim = vain.Categories.Blatant:CreateModule({
    	Name = 'Swim',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    local PlayerAttach
    PlayerAttach = vain.Categories.Blatant:CreateModule({
    	Name = 'Player Attach',
    	Tooltip = 'Attaches you to the nearest target',
    	Function = function(callback)
    		if callback then
    			repeat
    				if entitylib.isAlive then
    					...
    					if ent then
    						local root = entitylib.character.RootPart
    						rayCheck.FilterDescendantsInstances = { ent.Character, lplr.Character }
    						root.AssemblyLinearVelocity =
    							Vector3.new(0, root.Size.Y / 2 + entitylib.character.HipHeight + 0.75, 0)
    						root.CFrame = ent.RootPart.CFrame
    							+ (
    								not workspace:Raycast(ent.RootPart.Position, ent.RootPart.CFrame.LookVector, rayCheck)
    									and (ent.RootPart.CFrame.LookVector * 1.4)
    								or Vector3.zero
    							)
    					end
    				end
...
run(function()
    ...
    local function Added(ent)
    	...
    	arrow.Visible = false
    	arrow.Image = getcustomasset('vain/assets/new/arrowmodule.png')
    ...
    Arrows = vain.Categories.Render:CreateModule({
    	Name = 'Arrows',
    	Function = function(callback)
    		if callback then
    			...
    			Arrows:Clean(vain.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
    ...
    Chams = vain.Categories.Render:CreateModule({
    	Name = 'Chams',
    	Function = function(callback)
    		if callback then
    			...
    			Chams:Clean(vain.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
    ...
    ESP = vain.Categories.Render:CreateModule({
    	Name = 'ESP',
    	Function = function(callback)
    		if callback then
    			...
    			if ColorFunc[methodused] then
    				ESP:Clean(vain.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
    ...
    GamingChair = vain.Categories.Render:CreateModule({
    	Name = 'Gaming Chair',
    	Function = function(callback)
    		if callback then
    			if vain.ThreadFix then
    			...
    			--movingsound.SoundId = downloadVapeAsset('vain/assets/ChairRolling.mp3')
    			movingsound.Volume = 0.4
    			movingsound.Looped = true
    			movingsound.Parent = workspace
    			flyingsound = Instance.new('Sound')
    			--flyingsound.SoundId = downloadVapeAsset('vain/assets/ChairFlying.mp3')
    			...
    			repeat
    				if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
    					...
    					local flying = vain.Modules.Fly and vain.Modules.Fly.Enabled
    						or vain.Modules.LongJump and vain.Modules.LongJump.Enabled
    						or vain.Modules.InfiniteFly and vain.Modules.InfiniteFly.Enabled
...
run(function()
    ...
    Search = vain.Categories.Render:CreateModule({
    	Name = 'Search',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    local function Loop()
    	local screenSize = vain.gui.AbsoluteSize
    ...
    Tracers = vain.Categories.Render:CreateModule({
    	Name = 'Tracers',
    	Function = function(callback)
    		if callback then
    			...
    			Tracers:Clean(vain.Categories.Friends.ColorUpdate.Event:Connect(function()
    				ColorFunc(Color.Hue, Color.Sat, Color.Value)
    			end))
...
run(function()
    ...
    ZoomUnlocker = vain.Categories.Render:CreateModule({
    	Name = 'Zoom Unlocker',
...
run(function()
    ...
    ServerHop:CreateButton({
    	...
    	Function = function()
    		notif(
    			'ServerHop',
    			shared.vainserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server',
    			5
    		)
...
run(function()
    ...
    PromptDuration = vain.Categories.World:CreateModule({
...
run(function()
    ...
    PromptExtender = vain.Categories.World:CreateModule({
    			...
    			entitylib.getEntityColor = function(ent)
    				...
    				if isFriend(ent, true) then
    					return Color3.fromHSV(
    						vain.Categories.Friends.Options['Friends color'].Hue,
    						vain.Categories.Friends.Options['Friends color'].Sat,
    						vain.Categories.Friends.Options['Friends color'].Value
    					)
    				end
...
run(function()
    ...
    Coords = vain.Legit:CreateModule({
    	Name = 'Coords',
    	Category = 'Hud',
    ...
    for i, v in { 'x', 'y', 'z' } do
    	...
    	shadow.Parent = gui
    	vain:Clean(display:GetPropertyChangedSignal('Text'):Connect(function()
...
run(function()
    ...
    local function ChangeFFlag(suc)
    	...
    	if not success or typeof(json) ~= 'table' then
    		notif('Vain', 'Invalid json format for fflag', 12, 'warning')
    		return
    	end
    	...
    	notif('Vain', 'FFlags applied, Go in a new game to take effect', 12, 'info')
    end

    FFlag = vain.Legit:CreateModule({
    	Name = 'FFlag Editor',
    	...
    	Function = function(call)
    		if call then
    			...
    			notif('Vain', 'Inorder to disable fflags you have applied, You need to restart roblox', 20, 'info')
    		end
    	end,
    })
...
run(function()
    ...
    Memory = vain.Legit:CreateModule({
    	Name = 'Memory',
    	Category = 'Hud',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    Speedmeter = vain.Legit:CreateModule({
    	Name = 'Speedmeter',
    	Category = 'Hud',
    	Function = function(callback)
    		if callback then
...
-- BedWars (GameId 2619619496) spans many places: the main game, ranked, doubles,
-- practice, custom/private matches and more. Only 6872274481 and a couple of
-- match-redirect stubs ship a per-place file, so on every other BedWars match
-- place the full module set never loaded. Detect BedWars by GameId and load the
-- big module file directly. The lobby (6872265039) has its own file and does not
-- need the match modules. The big file self-guards against double-loading, so any
