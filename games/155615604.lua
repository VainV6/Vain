...
run(function()
	...
	if not pl.Bullet then
		repeat
			getShootFunction()
			task.wait()
		until pl.Bullet or vain.Loaded == nil
	...
	vain:Clean(replicatedStorage.Killfeed.ChildAdded:Connect(SessionAdded))
...
run(function()
    ...
    AntiKillPlane = vain.Categories.Blatant:CreateModule({
...
run(function()
    ...
    AntiTaze = vain.Categories.Blatant:CreateModule({
        Name = 'Anti Taze',
        Function = function(callback)
            if callback then
...
run(function()
    ...
    VehicleFly = vain.Categories.Blatant:CreateModule({
        Name = 'Vehicle Fly',
        Function = function(callback)
            if callback then
                up, down = 0, 0
...
run(function()
    ...
    VehicleSpeed = vain.Categories.Blatant:CreateModule({
        Name = 'Vehicle Speed',
        Function = function(callback)
            if callback then
                repeat
...
run(function()
    ...
    AutoDetonate = vain.Categories.Utility:CreateModule({
        Name = 'Auto Detonate',
        Function = function(callback)
            if callback then
...
run(function()
    ...
    AutoHeal = vain.Categories.Utility:CreateModule({
        Name = 'Auto Heal',
        Function = function(callback)
            if callback then
                repeat
